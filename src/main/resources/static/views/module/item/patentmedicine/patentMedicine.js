...
layui.use(['form', 'jquery', 'layer', 'ajax','utils'], function () {
    ...
    var rootMapping = '/item/patentMedicine';
    var leftTreeId = 'catalog';
    var formId = 'patentmedicine-form';
    var keyword = '';
    form.render();
    ...
    utils.splicingOption({
        ...
        where: {dictKey: DICT_KEY.ITEM_PATENT_MEDICINE},
        tips: '请选择所属分类'
    });
        ...
        if (utils.isNotNull(id)) {
            $.getJSON(rootMapping + '/findById', {id: id}, function (result) {
                if (result != null && utils.isNotNull(result.itemId)) {
                    ...
                    layer.alert('未找到该成药，请重试！',{icon: 0}, function (index) {
                        queryCatalog();
                        utils.clearForm('#' + formId);// 清空表单
                        form.render();
                        layer.close(index);
    ...
    var eventFunction = {
        ...
        delBtn: function () {
            ...
            if (utils.isNotEmpty(selectNodes) && !selectNodes[0].isParent) {
                layer.confirm(MSG.delete_confirm + '该成药吗？', {icon: 3}, function () {
                ...
                layer.msg('请选择一个成药！', {icon: LAYER_ICON.warning});
            }
        }
    };
