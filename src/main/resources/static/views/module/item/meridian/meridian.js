...
layui.use(['form', 'jquery', 'layer', 'ajax','utils'], function () {
    ...
    var rootMapping = '/item/meridian';
    var leftTreeId = 'catalog';
    var formId = 'meridian-form';
    var keyword = '';
    form.render();
        ...
        if (utils.isNotNull(id)) {
            $.getJSON(rootMapping + '/findById', {id: id}, function (result) {
                if (result != null && utils.isNotNull(result.itemId)) {
                    ...
                    layer.alert('未找到该经络，请重试！',{icon: 0}, function (index) {
                        queryCatalog();
                        utils.clearForm('#' + formId);// 清空表单
                        form.render();
                        layer.close(index);
    ...
    var eventFunction = {
        ...
        delBtn: function () {
            ...
            if (utils.isNotEmpty(selectNodes) && !selectNodes[0].isParent) {
                layer.confirm(MSG.delete_confirm + '该经络吗？', {icon: 3}, function () {
                ...
                layer.msg('请选择一个经络！', {icon: LAYER_ICON.warning});
            }
        }
    };
